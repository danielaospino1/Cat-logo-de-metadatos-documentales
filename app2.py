...
# Penalizaciones por "resolver" algo que estaba bien (trampas)
PENALIZACIONES = {
    "trampa_idioma":      -15,   # El idioma es homogéneo y correcto — no hay problema
    "trampa_estado":      -15,   # Los estados documentales son válidos por diseño archivístico
    "trampa_calificacion_drop": -20, # Eliminar registros con baja calificación no es limpieza de datos
}
PUNTOS_MAX = sum(PUNTOS.values())  # máximo alcanzable (sin penalizaciones)
...
def init_state():
    defaults = {
        ...
        "trampas_activadas": set(),   # trampas en las que cayó el participante
    }
...
def penalizar_trampa(clave: str):
    """Aplica penalización la primera vez que se activa una trampa."""
    if clave not in st.session_state.trampas_activadas:
        pts = PENALIZACIONES.get(clave, 0)
        st.session_state.puntos += pts   # pts es negativo
        st.session_state.trampas_activadas.add(clave)
        return pts
    return 0
...
def fase_limpiar():
    ...
    st.markdown(
        "Analiza cada caso con cuidado. **No todos los campos requieren intervención** — "
        "actuar sobre datos que ya están bien es tan perjudicial como ignorar errores reales. "
        "Cada acción incorrecta tiene penalización.")

    # ── Leyenda de tipos ──
    st.markdown("""
    <div style="display:flex;gap:10px;flex-wrap:wrap;margin-bottom:16px;">
      <span style="background:#0a2a0a;border:1px solid #10b981;border-radius:6px;
                   padding:4px 12px;font-size:0.8rem;color:#10b981;">⚠️ Requiere corrección — suma puntos</span>
      <span style="background:#2a0a0a;border:1px solid #ef4444;border-radius:6px;
                   padding:4px 12px;font-size:0.8rem;color:#ef4444;">🪤 Trampa — datos correctos, no intervenir</span>
    </div>
    """, unsafe_allow_html=True)
    ...
    st.markdown('<div class="section-title">📊 Estado actual del catálogo</div>',
                unsafe_allow_html=True)
    col1, col2, col3, col4 = st.columns(4)
    penalizacion_total = sum(PENALIZACIONES[k] for k in st.session_state.trampas_activadas)
    metricas = [
        (len(df), "Registros", "#0062cc"),
        (int(df.isnull().sum().sum()), "Metadatos faltantes", "#ef4444"),
        (int(df.duplicated().sum()), "Duplicados", "#f5a623"),
        (st.session_state.puntos, "Puntos acumulados", "#00c896" if st.session_state.puntos >= 0 else "#ef4444"),
    ]
    for col, (val, lbl, color) in zip([col1, col2, col3, col4], metricas):
        with col:
            st.markdown(f"""
    ...
    if st.session_state.trampas_activadas:
        st.error(f"🪤 Caíste en {len(st.session_state.trampas_activadas)} trampa(s) — penalización acumulada: {penalizacion_total} pts")

    st.markdown("---")

    # ── PROBLEMAS REALES (requieren corrección) ──
    st.markdown('<div class="section-title">⚠️ Casos para analizar — algunos requieren corrección</div>',
                unsafe_allow_html=True)

    problemas = [
        {
            "id": "limpiar_col_vacia",
            "titulo": "Campo completamente vacío: 'observaciones'",
        ...
        },
        {
            ...
            "titulo": "Títulos de documento faltantes",
        ...
        },
        {
            ...
            "titulo": "Dependencia productora faltante",
        ...
        },
        {
            ...
            "titulo": "Calidad de imagen sin registrar",
        ...
        },
        {
            ...
            "titulo": "Formato de archivo faltante",
        ...
        {
            "id": "trampa_idioma",
            "titulo": "🔤 ¿El campo 'idioma' necesita corrección?",
            "descripcion": (
                "El campo `idioma` parece tener un único valor repetido ('es'). "
                "Alguien del equipo propone eliminarlo por considerarlo redundante o re-codificarlo. "
                "¿Deberías intervenir?"
            ),
            "pistas_trampa": (
                "Revisa: `df['idioma'].value_counts()`. El campo tiene un solo valor — "
                "pero eso no es un error. Documentar el idioma es un metadato de identidad "
                "obligatorio en RiC-CM, incluso si todos los documentos son en español. "
                "Eliminarlo o 'limpiar' este campo es incorrecto."
            ),
            "concepto_trampa": (
                "Un campo con valor único y consistente NO es un problema de calidad de datos. "
                "En RiC-CM, el idioma es metadato de identidad obligatorio. Eliminarlo afectaría "
                "la interoperabilidad internacional del catálogo (p. ej., en redes como Europeana o AtoM). "
                "La homogeneidad aquí es una virtud, no un defecto."
            ),
            "penalizacion": PENALIZACIONES["trampa_idioma"],
            "es_trampa": True,
        },
        {
            "id": "trampa_estado",
            "titulo": "📋 ¿Los valores de 'estado' son inconsistentes?",
            "descripcion": (
                "La columna `estado` contiene cuatro valores: 'Activo', 'Inactivo', 'Transferido', 'Eliminado'. "
                "Un colega dice que 'Eliminado' no debería estar en el catálogo y propone filtrar esos registros."
            ),
            "pistas_trampa": (
                "Revisa: `df['estado'].value_counts()`. Los cuatro estados son válidos por diseño. "
                "En gestión documental, un documento 'Eliminado' del sistema sigue teniendo que tener "
                "registro en el catálogo (principio de trazabilidad). Eliminar esos registros "
                "destruiría evidencia de qué se desechó y cuándo."
            ),
            "concepto_trampa": (
                "El principio de trazabilidad archivística exige conservar el registro de los documentos "
                "eliminados. Su ausencia del catálogo no significa que no existieron. Filtrar registros "
                "por estado 'Eliminado' viola la cadena de custodia y puede tener implicaciones legales. "
                "Este campo está correcto tal como está."
            ),
            "penalizacion": PENALIZACIONES["trampa_estado"],
            "es_trampa": True,
        },
        {
            "id": "trampa_calificacion_drop",
            "titulo": "⭐ ¿Hay que eliminar documentos con baja calidad de imagen?",
            "descripcion": (
                "La columna `calidad_imagen` tiene valores entre 1 y 5. "
                "Alguien sugiere eliminar del catálogo todos los registros con calificación 1 "
                "porque 'esos documentos son inútiles para la IA'."
            ),
            "pistas_trampa": (
                "Revisa: `df['calidad_imagen'].value_counts()`. Los valores 1-5 son todos legítimos. "
                "Calidad baja (1) no significa dato erróneo — significa que ese documento necesita "
                "ser re-digitalizado. Eliminarlo del catálogo haría invisible esa necesidad."
            ),
            "concepto_trampa": (
                "Eliminar registros por criterio de calidad baja es una decisión archivística, no "
                "una operación de limpieza de datos. En preservación digital, los documentos con "
                "baja calidad de imagen son los que más necesitan atención. Su registro en el catálogo "
                "es precisamente lo que permite priorizar acciones de conservación. Confundir "
                "'dato de calidad baja' con 'dato erróneo' es un error conceptual frecuente."
            ),
            "penalizacion": PENALIZACIONES["trampa_calificacion_drop"],
            "es_trampa": True,
        },
    ]

    # ── Renderizar todos los casos mezclados ──
    for prob in problemas:
        prob_id    = prob["id"]
        es_trampa  = prob["es_trampa"]

        if es_trampa:
            ya_caido = prob_id in st.session_state.trampas_activadas
            if ya_caido:
                # Trampa ya activada — mostrar como error cometido
                border_color = "#7f1d1d"
                bg_color     = "#2a0a0a"
                icon         = "🪤"
                badge_html   = f'<span style="background:#7f1d1d;color:#fca5a5;border-radius:20px;padding:3px 12px;font-size:0.78rem;font-weight:700;margin-left:8px;">{prob["penalizacion"]} pts ¡trampa!</span>'
            else:
                border_color = "#92400e"
                bg_color     = "#1a0f00"
                icon         = "🔎"
                badge_html   = '<span style="background:#451a03;color:#fcd34d;border-radius:20px;padding:3px 12px;font-size:0.78rem;font-weight:700;margin-left:8px;">Analizar antes de actuar</span>'

            st.markdown(f"""
            <div style="background:{bg_color};border-left:4px solid {border_color};
                        border-radius:8px;padding:14px 18px;margin:8px 0;">
              <strong>{icon} {prob['titulo']}</strong>{badge_html}
              <p style="color:#c4a35a;margin:6px 0 0 0;font-size:0.9rem;">{prob['descripcion']}</p>
            </div>
            """, unsafe_allow_html=True)

            expander_label = f"🔎 Analizar: {prob['titulo']}"
            with st.expander(expander_label):
                st.markdown(f"""
                <div class="info-box" style="border-left-color:#f5a623;">
                🔍 <strong>Pista de análisis:</strong> {prob['pistas_trampa']}
                </div>
                """, unsafe_allow_html=True)
                _render_trampa(prob_id, prob["concepto_trampa"], prob["penalizacion"])

        else:
            ya_resuelto = prob_id in st.session_state.acciones_realizadas
            border_color = "#10b981" if ya_resuelto else "#f5a623"
            bg_color     = "#0a2a0a" if ya_resuelto else "#1e293b"
            icon         = "✅" if ya_resuelto else "⚠️"
            badge_html   = f'<span class="badge">+{prob["puntos"]} pts</span>'
            ...
            if not ya_resuelto:
                with st.expander(f"🔧 Resolver: {prob['titulo']}"):
                    ...
                    """, unsafe_allow_html=True)
                    _render_accion(prob_id, df)

    # ── Contador y botón de avance ──
    st.markdown("---")
    prob_reales   = [p for p in problemas if not p["es_trampa"]]
    completados   = len([p for p in prob_reales if p["id"] in st.session_state.acciones_realizadas])
    trampas_caidas = len(st.session_state.trampas_activadas)

    col_i, col_w = st.columns([2, 1])
    with col_i:
        st.info(f"📊 **{completados}/{len(prob_reales)} correcciones aplicadas** · {trampas_caidas} trampa(s) activada(s)")
    with col_w:
        if trampas_caidas > 0:
            st.warning(f"⚠️ Penalización: {sum(PENALIZACIONES[k] for k in st.session_state.trampas_activadas)} pts")
    ...
    if completados >= 5:
        ...
        st.warning("Aplica al menos 5 correcciones para ver tu resultado final.")


def _render_trampa(trap_id: str, concepto: str, penalizacion: int):
    """Renderiza la acción de una trampa — si el participante hace clic, pierde puntos."""
    ya_caido = trap_id in st.session_state.trampas_activadas

    if ya_caido:
        st.error(f"🪤 Ya aplicaste esta acción incorrecta. Penalización: {penalizacion} pts")
        st.markdown(f"""
        <div class="info-box" style="border-left-color:#ef4444;">
        📚 <strong>Por qué era una trampa:</strong> {concepto}
        </div>
        """, unsafe_allow_html=True)
    else:
        st.markdown(f"""
        <div class="info-box" style="border-left-color:#ef4444;">
        📚 <strong>Razón conceptual:</strong> {concepto}
        </div>
        """, unsafe_allow_html=True)

        col_ok, col_trap = st.columns(2)
        with col_ok:
            st.success("✅ Decisión correcta: **No intervenir** — este campo está bien.")
            st.markdown("_Si has leído el análisis y decides no actuar, estás en lo correcto._")
        with col_trap:
            st.warning(f"⚠️ Si haces clic abajo de todas formas: **{penalizacion} puntos**")
            if st.button(f"⚡ Aplicar corrección de todas formas", key=f"trap_btn_{trap_id}",
                         help="¡Cuidado! Esto es incorrecto y tiene penalización"):
                pts = penalizar_trampa(trap_id)
                st.error(f"🪤 ¡Trampa activada! {pts} puntos. Este campo estaba correcto.")
                st.rerun()
...
def fase_resultado():
    ...
    # ── ACCIONES ──
    st.markdown('<div class="section-title">✅ Correcciones aplicadas</div>',
                unsafe_allow_html=True)
    ...
    # ── TRAMPAS ──
    if st.session_state.trampas_activadas or True:  # siempre mostrar la sección
        st.markdown('<div class="section-title">🪤 Trampas — ¿qué decidiste?</div>',
                    unsafe_allow_html=True)
        trampas_info = [
            ("trampa_idioma",            "Campo 'idioma' — homogéneo y correcto"),
            ("trampa_estado",            "Campo 'estado' — valores válidos por diseño"),
            ("trampa_calificacion_drop", "Calidad 1-5 — datos de baja calidad ≠ datos erróneos"),
        ]
        col_t1, col_t2 = st.columns(2)
        for i, (tid, tlabel) in enumerate(trampas_info):
            caido = tid in st.session_state.trampas_activadas
            pen   = PENALIZACIONES[tid]
            icon  = "🪤" if caido else "🏅"
            color = "#7f1d1d" if caido else "#064e3b"
            texto = f"{pen} pts — caíste en la trampa" if caido else "Bien — no interveniste"
            target = col_t1 if i % 2 == 0 else col_t2
            with target:
                st.markdown(
                    f'<div style="background:#0a1f38;border-radius:8px;padding:10px 14px;'
                    f'margin:4px 0;border-left:3px solid {"#ef4444" if caido else "#00c896"};">'
                    f'{icon} <strong>{tlabel}</strong><br>'
                    f'<span style="color:{"#fca5a5" if caido else "#6ee7b7"};font-size:0.85rem;">{texto}</span>'
                    f'</div>', unsafe_allow_html=True)
    ...
    if st.button("🔄 Nueva auditoría", use_container_width=False):
        for key in ["df_trabajo", "df_original", "puntos",
                    "logros", "fase", "acciones_realizadas", "trampas_activadas"]:
