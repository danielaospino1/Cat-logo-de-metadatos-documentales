...
<style>
  ...
  @media (max-width: 768px) {
    ...
    .metric-card .lbl  { font-size: 0.8rem; }
    .info-box { font-size: 0.88rem !important; }
    .stButton > button {
      min-height: 48px;
      font-size: 1rem !important;
      padding: 10px 16px !important;
    }
    .badge { display: block; margin: 6px 0 0 0; width: fit-content; }
    .section-title { font-size: 1.1rem; }
    .hero-band { padding: 20px 16px !important; }
    .concept-pill { font-size: 0.75rem; padding: 3px 10px; }
...
def fase_limpiar():
    ...
    st.markdown(
        "Tienes **14 casos** para analizar. **11 requieren corrección** y "
        "**3 están bien tal como están** — intervenirlos tiene penalización. "
        "Lee cada caso con cuidado antes de actuar.")
    ...
    problemas = [
        ...
        {
            "id": "trampa_idioma",
            "titulo": "El campo 'idioma' tiene un único valor repetido",
            "descripcion": (
                "El campo `idioma` contiene solo el valor 'es' en todos los registros. "
                "Un colega del equipo propone eliminarlo del catálogo por considerarlo redundante. "
                "¿Requiere intervención?"
            ),
            "pistas_trampa": (
                "Analiza: `df['idioma'].value_counts()`. El campo tiene un solo valor, pero eso no es un error. "
                "El idioma es metadato de identidad obligatorio en RiC-CM aunque todos los documentos "
                "sean en español. La homogeneidad es una virtud, no un defecto."
            ),
            "concepto_trampa": (
                ...
                "la interoperabilidad internacional del catálogo (por ejemplo, en redes como Europeana o AtoM). "
                "La homogeneidad aquí es correcta."
            ),
        ...
        # 8 — 🪤 TRAMPA: estado 'Eliminado'
        {
            "id": "trampa_estado",
            "titulo": "Registros con estado 'Eliminado' en el catálogo",
            "descripcion": (
                "La columna `estado` incluye documentos marcados como 'Eliminado'. "
                "Un colega argumenta que esos registros no deberían estar en el catálogo "
                "activo y propone filtrarlos. ¿Están mal?"
            ),
            "pistas_trampa": (
                "Revisa: `df['estado'].value_counts()`. Los cuatro estados — Activo, Inactivo, Transferido, Eliminado — "
                "son válidos por diseño. En gestión documental, un documento 'Eliminado' sigue "
                "requiriendo registro para garantizar trazabilidad. Filtrarlos destruiría evidencia."
            ),
            "concepto_trampa": (
                "El principio de trazabilidad archivística exige conservar el registro de los documentos eliminados. "
                "Su presencia en el catálogo no significa que sigan disponibles — significa que existieron y que "
                "hubo una decisión de eliminación. Filtrarlos viola la cadena de custodia y puede tener implicaciones legales."
            ),
        ...
        # 12 — 🪤 TRAMPA: eliminar calidad_imagen baja
        {
            "id": "trampa_calificacion_drop",
            "titulo": "Documentos con calidad de imagen muy baja (valor 1)",
            "descripcion": (
                "La columna `calidad_imagen` registra valores del 1 al 5. "
                "Alguien propone eliminar todos los documentos con calificación 1 "
                "argumentando que 'son inútiles para los modelos de IA'. ¿Es correcto?"
            ),
            "pistas_trampa": (
                "Verifica: `df['calidad_imagen'].value_counts()`. Los valores 1-5 son todos legítimos. "
                "Calidad baja no significa dato erróneo — significa que ese documento necesita "
                "ser re-digitalizado. Eliminarlo del catálogo haría invisible esa necesidad."
            ),
            "concepto_trampa": (
                "Eliminar registros por calidad baja es una decisión archivística, no limpieza de datos. "
                "En preservación digital, los documentos con baja calidad de imagen son precisamente los que "
                "más atención requieren. Su presencia en el catálogo permite priorizar acciones de conservación. "
                "Confundir 'dato de calidad baja' con 'dato erróneo' es uno de los errores más frecuentes al "
                "aplicar IA a archivos."
            ),
    ...
    for prob in problemas:
        ...
        if es_trampa:
            ...
            if ya_caido:
                ...
                bg_col     = "#2a0a0a"
                desc_color = "#fca5a5"
                badge      = f'<span style="background:#7f1d1d;color:#fca5a5;border-radius:20px;padding:3px 12px;font-size:0.78rem;font-weight:700;margin-left:8px;">{prob["penalizacion"]} pts</span>'
            else:
                icon       = "⚠️"
                border_col = "#f5a623"   # idéntico a tarjeta real pendiente
                bg_col     = "#1e293b"
                desc_color = "#6fa3d4"
                badge      = '<span class="badge">? pts</span>'

            st.markdown(f"""
            <div style="background:{bg_col};border-left:4px solid {border_col};
                        border-radius:8px;padding:14px 18px;margin:8px 0;">
              <strong>{icon} {prob['titulo']}</strong>{badge}
              <p style="color:{desc_color};margin:6px 0 0 0;font-size:0.9rem;">{prob['descripcion']}</p>
            </div>
            """, unsafe_allow_html=True)

            if not ya_caido:
                with st.expander(f"🔎 Analizar: {prob['titulo']}"):
                    st.markdown(f"""
                    <div class="info-box" style="border-left-color:#f5a623;">
                    🔍 <strong>Pista:</strong> {prob['pistas_trampa']}
                    </div>
                    """, unsafe_allow_html=True)
                    _render_trampa(prob_id, prob["concepto_trampa"], prob["penalizacion"])
            else:
                with st.expander(f"🪤 Ver explicación: {prob['titulo']}"):
                    st.markdown(f"""
                    <div class="info-box" style="border-left-color:#ef4444;">
                    📚 <strong>Por qué era una trampa:</strong> {prob['concepto_trampa']}
                    </div>
                    """, unsafe_allow_html=True)
